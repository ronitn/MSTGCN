...
def F_norm_loss(S, Falpha):
    ...
    if len(S.shape) == 4:
        ...
        return Falpha * K.sum(K.square(S))
...
class cheb_conv_with_Att_GL(Layer):
    ...
    def call(self, x):
        ...
        for time_step in range(T):
            # shape of x is (batch_size, V, F)
            graph_signal = x[:, time_step, :, :]
            batch_size = tf.shape(x)[0]  # Extract batch size first
            output = tf.zeros((batch_size, V, self.num_of_filters), dtype=tf.float32)
...
#         self.num_of_filters = num_of_filters
#        # self.cheb_polynomials = tf.to_float(cheb_polynomials)
#         self.cheb_polynomials = tf.cast(cheb_polynomials, dtype=tf.float32)
...
#     #         graph_signal = x[:, time_step, :, :]
#     #         # output = K.zeros(shape=(tf.shape(x)[0], V, self.num_of_filters))
#     #         output = tf.zeros_like(x[:, :, :self.num_of_filters])
...
#     #             #output = output + K.dot(rhs, theta_k)
#     #             rhs = K.batch_dot(tf.transpose(T_k_with_at, perm=[0, 2, 1]), graph_signal)
#     #             rhs = tf.reshape(rhs, [-1, tf.shape(rhs)[1], 256])  # Ensure correct shape
#     #             rhs = tf.cast(rhs, dtype=tf.float32)
#     #             theta_k = tf.cast(theta_k, dtype=tf.float32)

#     #             output = tf.zeros((tf.shape(rhs)[0], tf.shape(rhs)[1], 64), dtype=tf.float32)  # Explicitly set shape

#     #             output = K.dot(rhs, theta_k)  # Matrix multiplication
#     #             print("Updated output shape:", output.shape)  # Debugging output
...
#         x, Att = x  # Unpack input
#         batch_size = tf.shape(x)[0]  # Extract batch size
#         _, T, V, F = x.shape
...
#             # ✅ Fix: Initialize output properly
#             output = tf.zeros((batch_size, V, self.num_of_filters), dtype=tf.float32)

#             for kk in range(self.k):
#                 T_k = self.cheb_polynomials[kk]  # (V, V)
#                 T_k_with_at = K.dropout(T_k * Att, 0.6)  # (batch_size, V, V)
#                 theta_k = self.Theta[kk]  # (F, num_of_filters)

#                 rhs = K.batch_dot(tf.transpose(T_k_with_at, perm=[0, 2, 1]), graph_signal)
#                 rhs = tf.reshape(rhs, [batch_size, tf.shape(rhs)[1], 256])
#                 rhs = tf.cast(rhs, dtype=tf.float32)
#                 theta_k = tf.cast(theta_k, dtype=tf.float32)

#                 # ✅ Fix: Use direct assignment instead of in-place addition
#                 output = output + K.dot(rhs, theta_k)  

#                 print(f"Iteration {kk}: rhs shape: {rhs.shape}, theta_k shape: {theta_k.shape}, output shape: {output.shape}")
...
#     def get_config(self):  # ✅ Add this function
...
class cheb_conv_with_Att_static(Layer):
    ...
    def __init__(self, num_of_filters, k, cheb_polynomials, dropout_rate=0.6, **kwargs):
        super(cheb_conv_with_Att_static, self).__init__(**kwargs)
        self.k = k
        self.num_of_filters = num_of_filters
        self.cheb_polynomials = [tf.convert_to_tensor(poly, dtype=tf.float32) for poly in cheb_polynomials]
        self.dropout_rate = dropout_rate  # ✅ Added dropout rate as an attribute

    def build(self, input_shape):
        assert isinstance(input_shape, list)
        x_shape, _ = input_shape  # Only need the x_shape
        _, _, V, F = x_shape
    ...
    def call(self, inputs):
        assert isinstance(inputs, list) and len(inputs) == 2, 'cheb_gcn error'
        
        x, Att = inputs  # Unpack inputs
        ...
        for time_step in range(T):
            ...
            for kk in range(self.k):
                T_k = self.cheb_polynomials[kk]  # (V, V)
                T_k_with_at = K.dropout(T_k * Att, self.dropout_rate)  # Apply dropout
                theta_k = self.Theta[kk]  # (F, num_of_filters)

                rhs = K.batch_dot(tf.transpose(T_k_with_at, perm=[0, 2, 1]), graph_signal)
                rhs = tf.reshape(rhs, [batch_size, tf.shape(rhs)[1], F])  # Ensure correct shape

                output += K.dot(rhs, theta_k)  # Perform matrix multiplication
    ...
    def get_config(self):  # ✅ Properly define attributes in get_config()
        config = super().get_config()
        config.update({
...
#     # relu_x = K.relu(x)
#     # ln = tf.contrib.layers.layer_norm(relu_x, begin_norm_axis=3)
#     relu_x = tf.keras.activations.relu(x)
#     ln = tf.keras.layers.LayerNormalization(axis=-1)(relu_x) 
#     return ln
layer_norm = tf.keras.layers.LayerNormalization(axis=-1)  # ✅ Create once

def LayerNorm(x):
    relu_x = tf.keras.activations.relu(x)
    ln = layer_norm(relu_x)  # ✅ Use the pre-created instance
    return ln
...
def reverse_gradient(X, hp_lambda):
    ...
    num_calls = 1  # Initialize call counter
    try:
        reverse_gradient.num_calls += 1
    except AttributeError:
        reverse_gradient.num_calls = num_calls
        num_calls += 1  # Increment for the first time

    @tf.custom_gradient
    def _flip_gradients(x):
        def grad(dy):
            return -hp_lambda * dy  # Flip the gradient
        
        return x, grad  # ✅ Return x so that y is correctly passed

    y = _flip_gradients(X)  # ✅ Apply custom gradient function
    return y  # ✅ Ensures function returns y
...
class GradientReversal(Layer):
    ...
    def build(self, input_shape):
        pass  # No need to modify trainable_weights
...
################################################################################################
# MSTGCN Block
def expected_shape(input_shapes):
    """
    Function to define the expected output shape of the Lambda layer.
    """
    x_shape, att_shape = input_shapes  # Unpacking input shapes
    return x_shape  # Adjust based on reshape_dot function output
...
def MSTGCN_Block(x, k, num_of_chev_filters, num_of_time_filters, time_conv_strides,
    ...
    temporal_Att = TemporalAttention()(x)
    #x_TAtt = Lambda(reshape_dot, name='reshape_dot'+str(i))([x, temporal_Att])
    #x_TAtt = Lambda(lambda inputs: reshape_dot(inputs), name='reshape_dot' + str(i))([x, temporal_Att])
    x_TAtt = Lambda(reshape_dot, output_shape=expected_shape, name='reshape_dot' + str(i))([x, temporal_Att])
   ...
   # x_TAtt = Lambda(reshape_dot, output_shape=expected_shape, name='reshape_dot' + str(i))([x, temporal_Att])
    ...
    # end_output_GL = Lambda(LayerNorm, name='layer_norm' + str(2*i))(time_conv_output_GL)
    # end_output_SD = Lambda(LayerNorm, name='layer_norm' + str(2*i+1))(time_conv_output_SD)
    end_output_GL = Lambda(LayerNorm, output_shape=lambda s: s, name='layer_norm' + str(2*i))(time_conv_output_GL)
    end_output_SD = Lambda(LayerNorm, output_shape=lambda s: s, name='layer_norm' + str(2*i+1))(time_conv_output_SD)
